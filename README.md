...
Compiling a Linux kernel natively on macOS is not an easy feat. For this reason, ```libkrunfw``` is built by leveraging [krunvm](https://github.com/slp/krunvm) and its dependencies ([libkrun](https://github.com/containers/libkrun), and ```libkrunfw``` itself), such as the one available in the [krunvm Homebrew repo](https://github.com/slp/homebrew-krun).
